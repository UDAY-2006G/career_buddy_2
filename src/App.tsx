...
function App() {
  ...
  const handleGetBtechRecommendations = async () => {
    if (!btechBranch.trim() || !btechInterests.trim()) {
      alert('Please fill in both B.Tech branch and interests');
      return;
    }

    if (!import.meta.env.VITE_GEMINI_API_KEY || import.meta.env.VITE_GEMINI_API_KEY === 'AIzaSyD1-EXAMPLE_KEY-DontUseInProd') {
      alert('Please configure a valid Gemini API key in the .env file');
      return;
    }

    setIsLoadingBtech(true);
    setBtechRecommendations([]);
    
    try {
      // Try with exponential backoff
      await withRetry(generateBtechRecommendations, {
        maxRetries: 3,
  ...
  const withRetry = async (fn, { maxRetries, initialDelay, maxDelay, backoffFactor }) => {
    let lastError;
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries - 1) {
          ...
          const delayTime = Math.min(delay, maxDelay);
  ...
  const generateBtechRecommendations = async () => {
    ...
    if (!apiKey || apiKey === 'AIzaSyD1-EXAMPLE_KEY-DontUseInProd') {
      ...
      <div className="max-w-4xl mx-auto h-screen flex flex-col pt-8 pb-4">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-2">
            {!import.meta.env.VITE_GEMINI_API_KEY || import.meta.env.VITE_GEMINI_API_KEY === 'AIzaSyD1-EXAMPLE_KEY-DontUseInProd' ? (
              <span className="text-yellow-600">⚠️ Please configure your Gemini API key in .env file</span>
            ) : (
              <span className="text-green-600">✅ AI services connected</span>
            )}
